...
import com.archer.admin.web.product.entities.ProductTransform.ProductStockRes;
...
public class BizProductService {
    ...
    public Result modify(ProductPostReq productPostReq) {
        handleProductPics(productPostReq.getId(), productPostReq.getPicMainUrls(),
                productPostReq.getPicIntroUrls());
    ...
    public ProductQueryRes list(ProductQueryReq productQueryReq) {
        Page<Product> page = productService.lambdaQuery()
                ...
                .eq(productQueryReq.getCopyrightId() != 0, Product::getCopyrightId,
                        productQueryReq.getCopyrightId())
        ...
        List<ProductRes> list = page.getRecords().stream()
                .map(product -> ProductRes.parseRes(product,
                        copyrightMap.getOrDefault(product.getCopyrightId(), new Copyright())
                                .getCopyrightName(),
    ...
    public Result settingSpecAndStock(WebContext webContext,
            ProductTransform.ProductSpecAggrReq productSpecAggrReq) {
            ...
            if (groupReq.getValues() != null) {
                ...
                for (ProductTransform.ProductSpecValueTransform valueReq : groupReq.getValues()) {
                    if (StringUtils.isNotBlank(valueReq.getId())) {
                        // 更新
                        ProductSpecValue specValue = productSpecValueService
                                .getById(valueReq.getId());
                        if (specValue != null) {
                        ...
                        // 新增
                        ProductSpecValue specValue = toSepcValue0(productId, groupId, valueReq,
    ...
    public List<ProductChannelConfigRes> queryProductChannelConfigList(WebContext webContext, String productId) {
        ...
        Map<Integer, ProductChannel> productChannelMap = productChannelService.lambdaQuery()
                ...
                .list().stream()
                .collect(Collectors.toMap(ProductChannel::getChannelId, Function.identity()));
        ...
        return channels.stream().map(channel -> {
            ...
            ProductChannelConfigRes res = ProductChannelConfigRes.builder()
                    ...
                    .stockStrategy(StockStrategyEnum
                            .of(Optional.ofNullable(thisChannel)
                                    .map(ProductChannel::getStockStrategy)
                                    .orElse(0)))
                    .platformProductId(
                            Optional.ofNullable(thisChannel)
                                    .map(ProductChannel::getPlatformProductId)
                                    .orElse(""))
                    .build();
    ...
    public List<ProductChannelSkuConfigRes> queryProductChannelSkuList(WebContext webContext, String productId,
        ...
        List<SkuRes> skuResList = bizSkuService.getSkuResList(skus);

        List<String> skuIds = skuResList.stream().map(SkuRes::getId).collect(Collectors.toList());
        ...
        Map<String, ProductChannelSku> productChannelSkus = productChannelSkuService.lambdaQuery()
                ...
                .list().stream()
                .collect(Collectors.toMap(ProductChannelSku::getSkuId, Function.identity()));

        return skuResList.stream().map(sku -> {
            ProductChannelSku productChannelSku = productChannelSkus.get(sku.getId());
            return ProductChannelSkuConfigRes.builder()
                    .id(Optional.ofNullable(productChannelSku).map(ProductChannelSku::getId)
                            .orElse(0))
                    .skuId(sku.getId())
                    .shareNum(110)
                    .lockNum(Optional.ofNullable(productChannelSku)
                            .map(ProductChannelSku::getLockNum).orElse(0))
                    .surplusNum(11)
                    .specCombination(sku.getSpecValues())
                    .build();
        }).collect(Collectors.toList());
    }
    ...
    public Result saveChannelSkuLockConfig(WebContext webContext,
        ...
        long count = productChannelService.lambdaQuery()
                .eq(ProductChannel::getPlatformProductId,
                        saveChannelSkuLockConfig.getPlatformProductId())
                .count();
        if (saveChannelSkuLockConfig.getId() > 0 || count > 0) {
            ...
            productChannelSkuService.lambdaUpdate()
                    ...
                    .eq(ProductChannelSku::getPlatformProductId,
                            saveChannelSkuLockConfig.getPlatformProductId())
                    .remove();
        } else {
    ...
    public ProductStockQueryRes stockList(ProductQueryReq productQueryReq) {
        ...
        // 查询库存
        Map<String, List<Sku>> skuMap = skuService.lambdaQuery()
                .in(Sku::getProductId, productIds)
                .list().stream()
                .collect(Collectors.groupingBy(Sku::getProductId));

        List<ProductStockRes> list = page.getRecords().stream()
                .map(product -> ProductStockRes.parseRes(product,
                        copyrightMap.getOrDefault(product.getCopyrightId(), new Copyright())
                                .getCopyrightName(),
                        picMap.getOrDefault(product.getId(), Lists.newArrayList()),
                        skuMap.getOrDefault(product.getId(), Lists.newArrayList())))
                .collect(Collectors.toList());

        return ProductStockQueryRes.builder()
                .total(page.getTotal())
                .totalPage(page.getPages())
                .list(list)
                .build();
